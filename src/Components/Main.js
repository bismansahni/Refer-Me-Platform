...
const Main = () => {
  ...
  const handleToggleDashboard = (component) => {
    console.log(`Switching to component: ${component}`);
    setActiveComponent(component);
    setShowUpdateProfile(false); // Ensure other components can be shown
  };

  const handleUpdateProfile = (data) => {
    setProfileDataToUpdate(data);
  ...
  return (
    <div className={MainStyles['main-container']}>
      ...
      <div className={MainStyles['right-dashboard']}>
        {showUpdateProfile ? (
          <UpdateProfile profileData={profileDataToUpdate} onCancel={() => setShowUpdateProfile(false)} />
        ) : (
          <>
            {activeComponent === 'Update Profile' && <UpdateProfile />}
            {activeComponent === 'My Requests' && <MyRequests />}
            {activeComponent === 'Be A Referrer' && <BeReferrer />}
            {activeComponent === 'Get Referred' && <GetReferred onUpdateProfile={handleUpdateProfile} />}
          </>
        )}
      </div>
    </div>
